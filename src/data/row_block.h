...
struct RowBlockContainer {
  ...
  /*! \brief array[size] timing of each instance */
  std::vector<real_t> timing;
  /*! \brief field index */
  ...
  inline void Clear(void) {
    ...
    label.clear(); field.clear(); index.clear(); value.clear(); weight.clear(); timing.clear();
    max_field = 0;
    max_index = 0;
  }
  ...
  inline void Push(Row<I> row) {
    ...
    timing.push_back(row.get_timing());
    if (row.field != NULL) {
  ...
  inline void Push(RowBlock<I> batch) {
    ...
    if (batch.timing != NULL) {
      timing.insert(timing.end(), batch.timing, batch.timing + batch.size);
    }
...
RowBlockContainer<IndexType>::Load(Stream *fi) {
  ...
  CHECK(fi->Read(&timing)) << "Bad RowBlock format";
